...
def compute_loss(p, targets, model): # predictions, targets, model
    ...
    for i, pi in enumerate(p): # layer index, layer predictions
        ...
        if n: # number of targets
            ...
            # Apply exponent to wh predictions and multiply with the anchor box 
            # that matched best with the label for each cell that has a target
            pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
            # Build box out of xy and wh
            pbox = torch.cat((pxy, pwh), 1).to(device)  # predicted box
            # Calculate CIoU or GIoU for each target with the predicted box for its cell + anchor
            iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
            # We want to minimize our loss so we and 
            # the best possible IoU is 1 so we take 1 - IoU and reduce it with a mean
            lbox += (1.0 - iou).mean()  # iou loss

            # classification of the objectness
            # Fill out empty object target tensor with the IoU we 
            # just calculated for each target at the targets position
